...
def write_wrong_output(args, task, prefix, preds, labels):
    ...
    output_pred_file = os.path.join(args.output_dir, prefix, "dev_preds.tsv")
...
def evaluate(args, model, tokenizer, prefix="", write_output=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_sampler = SequentialSampler(eval_dataset) if (args.local_rank == -1 or write_output) else DistributedSampler(eval_dataset)
        ...
        if write_output:
            write_wrong_output(args, eval_task, prefix, preds, out_label_ids)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            global_step = ""
            result = evaluate(args, model, tokenizer, write_output=True)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
